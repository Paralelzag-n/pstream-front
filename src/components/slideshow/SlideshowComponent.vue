...
const computedImageWidth = computed(()=>{
 if(slideshowContainerRef.value) return slideshowContainerWidth.value * slideshowImageWidth.value / 100
  return 0
})
...
const computedTranslateX = computed(()=>{
  return computedImageWidth.value * 2 - computedImageWidth.value * 0.315  + 32 + animateSlideshowTranslateX.value
})
...
<template>
  <div class="relative h-slideshow overflow-hidden" ref="slideshowContainerRef" @mouseenter="toggleHoveringSlideshow(true)" @mouseleave="toggleHoveringSlideshow(false)">
    <div class="h-slideshow flex absolute top-0 left-0 gap-4" :class="{'transition-all duration-700': transitionInProgress}" :style="{'transform': `translateX(${-computedTranslateX}px)`}">
      ...
      <!-- Main Image -->
      <SlideshowItemComponent :isScrolledPast="isScrolledPast" :slideshowChange="slideshowChange" @onVideoEnd="viewNextShow" @muteVideo="toggleMute" :mute-video="videosMuted" :key="props.shows[currentShow].name" class="shadow-2xl" :show="props.shows[currentShow]" :slideshow-image-width="slideshowImageWidth" :main-slideshow="true" :class="{'brightness-50 transition duration-500': animateSlideshowTranslateX !== 0, 'brightness-100':animateSlideshowTranslateX === 0}"/>
      <!-- Next Image -->
    ...
    <div @click="viewPreviousShow" style="backdrop-filter: blur(1rem);" :class="{'opacity-0': !slideShowHovering, 'opacity-100': slideShowHovering}" class="absolute top-1/2 duration-300 -translate-y-1/2 left-10 transition-all cursor-pointer bg-gradient-to-br from-indigo-500/50 to-indigo-700/40 hover:scale-110 text-white font-bold py-7 px-6 rounded-md shadow-md select-none"> <i class="fa-solid fa-chevron-left"></i> </div>
    <div @click="viewNextShow" style="backdrop-filter: blur(1rem);" :class="{'opacity-0': !slideShowHovering, 'opacity-100': slideShowHovering}" class="absolute top-1/2 duration-300 -translate-y-1/2 right-10 transition-all cursor-pointer bg-gradient-to-br from-indigo-500/40 to-indigo-700/50 hover:scale-110 text-white font-bold py-7 px-6 rounded-md shadow-md select-none"> <i class="fa-solid fa-chevron-right"></i> </div>
  </div>
</template>
